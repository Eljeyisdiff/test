...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    try {
        // Validate input
        if (!isset($_POST['office_name']) || !isset($_POST['office_prefix']) || !isset($_POST['office_description'])) {
        ...
        try {
            // Insert new office
            $query = "INSERT INTO offices (office_name, prefix, office_description, status) VALUES (?, ?, ?, 'closed')";
            $stmt = $conn->prepare($query);
            $stmt->bind_param('sss', $office_name, $prefix, $office_description);
            
            if (!$stmt->execute()) {
                throw new Exception('Failed to add new office');
            }

            $office_id = $stmt->insert_id;

            // Insert purposes if any exist
            if (!empty($purposes)) {
        ...
        error_log('Error in add_office.php: ' . $e->getMessage());
        echo json_encode([
            'success' => false,
            'error' => $e->getMessage()
        ]);
